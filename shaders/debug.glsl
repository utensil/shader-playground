...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float progress = ease((clamp((iTime - iTimeCursorChange) / DURATION, 0.0, 1.0)));
    ...
    float cCursorDistance = getSdfRectangle(vu, currentCursor.xy - (currentCursor.zw * offsetFactor), currentCursor.zw * 0.5);
    float d2 = getSdfParallelogram(vu, v0, v1, v2, v3);
    ...
    newColor = mix(newColor, TRAIL_COLOR, 1.0 - smoothstep(cCursorDistance, -0.000, 0.003 * (1. - progress)));

    // Mask out the cursor shape from the effect
